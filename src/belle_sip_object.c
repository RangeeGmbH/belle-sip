...
int belle_sip_object_unref_2(void *ptr) {
	...
	if (obj->vptr->initially_unowned && obj->ref == 0) {
		if (obj->pool) belle_sip_object_pool_remove(obj->pool, obj);
		obj->ref = -1;
	...
	if (obj->vptr->on_last_ref) {
		if ((obj->vptr->initially_unowned && obj->ref == 1) || (!obj->vptr->initially_unowned && obj->ref == 2)) {
...
belle_sip_object_t *
belle_sip_object_weak_ref(void *obj, belle_sip_object_destroy_notify_t destroy_notify, void *userpointer) {
	belle_sip_object_t *o = BELLE_SIP_OBJECT(obj);
...
static void _belle_sip_object_uninit(belle_sip_object_t *obj) {
	if (obj->name) belle_sip_free(obj->name);
}
...
_belle_object_marshal(belle_sip_object_t *obj, char *buff, size_t buff_size, size_t *offset) {
	return belle_sip_snprintf(buff, buff_size, offset, "{%s::%s %p}", belle_sip_object_vptr_get_type_name(obj),
	                          obj->name ? obj->name : "(no name)", obj);
}
...
belle_sip_object_vptr_t belle_sip_object_t_vptr = {BELLE_SIP_TYPE_ID(belle_sip_object_t),
...
static belle_sip_object_vptr_t *find_common_floor(belle_sip_object_vptr_t *vptr1, belle_sip_object_vptr_t *vptr2) {
	...
	for (it1 = vptr1; it1 != NULL; it1 = it1->get_parent()) {
		if (it1 == vptr2) return vptr2;
	}
	for (it2 = vptr2; it2 != NULL; it2 = it2->get_parent()) {
		if (vptr1 == it2) return vptr1;
	}
...
void _belle_sip_object_copy(belle_sip_object_t *newobj, const belle_sip_object_t *ref) {
	...
	while (vptr != NULL) {
		if (vptr->clone == NULL) {
			belle_sip_fatal("Object of type %s cannot be cloned, it does not provide a clone() implementation.",
			                vptr->type_name);
			return;
...
int belle_sip_object_data_set(belle_sip_object_t *obj,
                              const char *name,
                              void *data,
                              belle_sip_data_destroy destroy_func) {
	int ret = 0;
...
void belle_sip_object_data_clone(const belle_sip_object_t *src,
                                 belle_sip_object_t *dst,
                                 belle_sip_data_clone clone_func) {
	belle_sip_object_data_clear(dst);
	belle_sip_object_data_merge(src, dst, clone_func);
}

void belle_sip_object_data_merge(const belle_sip_object_t *src,
                                 belle_sip_object_t *dst,
                                 belle_sip_data_clone clone_func) {
...
void belle_sip_object_data_foreach(const belle_sip_object_t *obj,
                                   void (*apply_func)(const char *key, void *data, void *userdata),
                                   void *userdata) {
...
void *belle_sip_object_cast(
    belle_sip_object_t *obj, belle_sip_type_id_t id, const char *castname, const char *file, int fileno) {
	if (obj != NULL) {
...
void *belle_sip_object_interface_cast(
    belle_sip_object_t *obj, belle_sip_interface_id_t ifid, const char *castname, const char *file, int fileno) {
	if (obj != NULL) {
...
void belle_sip_object_set_name(belle_sip_object_t *object, const char *name) {
	...
	if (name) object->name = belle_sip_strdup(name);
}
...
static belle_sip_error_code
checked_marshal(belle_sip_object_vptr_t *vptr, belle_sip_object_t *obj, char *buff, size_t buff_size, size_t *offset) {
	...
	if (error == BELLE_SIP_OK) {
		if (written != (*offset - initial_offset) &&
		    written != (buff_size - initial_offset - 1)) { /*this is because snprintf won't allow you to write a non
			                                                  null character at the end of the buffer*/
			belle_sip_fatal("Object of type %s marshalled %i bytes but said it marshalled %i bytes !", vptr->type_name,
			                (int)written, (int)(*offset - initial_offset));
		}
		...
		if (written > buff_size) {
			belle_sip_fatal("Object of type %s commited a buffer overflow by marshalling %i bytes", vptr->type_name,
			                (int)(*offset - initial_offset));
		}
	} else {
		belle_sip_error("Object of type %s produced an error during marshalling: %i", vptr->type_name, error);
	}
	belle_sip_free(p);
	return error;
}
...
belle_sip_error_code belle_sip_object_marshal(belle_sip_object_t *obj, char *buff, size_t buff_size, size_t *offset) {
	...
	while (vptr != NULL) {
		if (vptr->marshal != NULL) {
			...
			else return vptr->marshal(obj, buff, buff_size, offset);
		} else {
			vptr = vptr->get_parent();
		}
	}
...
static char *belle_sip_object_to_alloc_string(const belle_sip_object_t *obj, int size_hint) {
	...
	if (error == BELLE_SIP_BUFFER_OVERFLOW) {
		belle_sip_message(
		    "belle_sip_object_to_alloc_string(): hint buffer was too short while doing to_string() for %s, retrying",
		    belle_sip_object_vptr_get_type_name(obj));
		belle_sip_free(buf);
...
char *belle_sip_object_to_string(const void *_obj) {
	...
	if (obj->vptr->tostring_bufsize_hint != 0) {
		...
		belle_sip_error_code error =
		    belle_sip_object_marshal((belle_sip_object_t *)obj, buff, sizeof(buff) - 1, &offset);
		if (error == BELLE_SIP_BUFFER_OVERFLOW) {
			belle_sip_message(
...
char *_belle_sip_object_describe_type(belle_sip_object_t *obj, belle_sip_object_vptr_t *vptr) {
	...
	for (elem = l; elem != NULL; elem = elem->next) {
		...
		if (elem->next) belle_sip_snprintf(ret, maxbufsize, &pos, "\t        |\n");
	}
...
char *belle_sip_object_describe_type_from_name(const char *name) {
	return belle_sip_strdup_printf(
...
BELLE_SIP_INSTANCIATE_VPTR(
    belle_sip_object_pool_t, belle_sip_object_t, belle_sip_object_pool_destroy, NULL, NULL, FALSE);
...
void belle_sip_object_pool_clean(belle_sip_object_pool_t *pool) {
	...
	if (!belle_sip_object_pool_cleanable(pool)) {
		belle_sip_warning(
		    "Thread pool [%p] cannot be cleaned from thread [%lu] because it was created for thread [%lu]", pool,
		    belle_sip_thread_self_id(), (unsigned long)pool->thread_id);
		return;
	}
...
belle_sip_object_pool_t *belle_sip_object_pool_get_current(void) {
	...
	if (*pools == NULL) {
		if (first_time) {
			belle_sip_warning(
			    "There is no object pool created in thread [%lu]. "
